...
const buttonVariants = cva(
	[
		"group relative rounded-full overflow-clip transition-all duration-300 font-semibold select-none",
	...
	return (
		<Component className={cn(buttonVariants({ variant, size }), className)} {...props}>
			...
			<div className="relative z-1">
				<span>{children}</span>
				{/* Inset shadow effect */}
				<span className="text-transparent absolute inset-0">{children}</span>
			</div>
		</Component>
	)
}
